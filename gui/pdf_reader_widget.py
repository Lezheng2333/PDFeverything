"""PDF reader — Scroll + Grid modes, label pool, pre-render, native pinch-zoom, close btn."""
...
RENDER_SCALE = 2; RESIZE_DEBOUNCE = 300


class PdfReaderWidget(QWidget):
    """Scroll-based PDF reader (continuously scrolls all pages) + thumbnail grid.
    Label pool reused forever. Pre-renders all pages on open."""
    ...
    def __init__(self, parent=None):
        ...
        self._zoom_mode = "fit_width"   # "fit_width" | "fit_height" | float
        ...
        self._pre_render_idx = 0  # batch progress
    ...
    def _init_ui(self):
        ...
        tb.addWidget(self.btn_scroll); tb.addWidget(self.btn_grid); tb.addSpacing(16)

        # Nav (always visible in scroll, hidden in grid)
        ...
        self.btn_fit_width = QPushButton("Fit Width"); self.btn_fit_width.setCheckable(True)
        ...
        self.btn_fit_height.setCheckable(True)
        ...
        self.btn_close.setToolTip("Close this document")
    ...
    def _set_mode(self, mode: ViewMode):
        ...
        self.nav_widget.setVisible(mode == ViewMode.SCROLL)
        if self.doc:
            self._layout_labels()
    ...
    def open_pdf(self, path: Path) -> None:
        ...
        if self.doc.is_encrypted:
            if not self.doc.authenticate(""):
                ...
                self._show_welcome("This PDF is password-protected."); return
    ...
    def zoom_fit_width(self):
        ...
        if self.doc: self._update_label_pixmaps(); self._layout_labels()
        self._start_pre_render()
    ...
    def _build_labels(self):
        for pi in range(self._total_pages):
            ...
            l.mousePressEvent = lambda ev: None  # eat single-click
    ...
    def _layout_labels(self):
        ...
        if self._view_mode == ViewMode.SCROLL:
            ...
            for pi, l in enumerate(self._labels):
                ...
                y += (pix.height() + spacing) if pix else (800 + spacing)
                max_w = max(max_w, pix.width() if pix else 600)
    ...
    def _on_grid_dbl_click(self, page_idx: int):
        """Double-click thumbnail → Fit Height + Scroll mode at that page."""
    ...
    def _pre_render_batch(self):
        """Render ~40 pages per batch, deferring to event loop between batches
        to keep UI responsive during initial load."""
        if not self.doc: return
        vw, vh = self._viewport_size()
        batch_size = 40
        ...
        if self._pre_render_idx < self._total_pages:
            self._pre_render_timer.start()  # schedule next batch
        else:
            # Apply pixmaps to labels
            self._update_label_pixmaps()

    # ═══════════════ Scroll / Page tracking ═══════════════

    def _scroll_to_page_top(self):
        """Scroll so that current_page's top aligns with viewport top."""
        if not self._labels or self._current_page >= len(self._labels):
            return
        l = self._labels[self._current_page]
        if not l or l.isHidden():
            return
        pos_y = l.pos().y()
        self.scroll_area.verticalScrollBar().setValue(max(0, pos_y))

    def _track_scroll_page(self):
        """Determine which page occupies >50% of viewport and update indicator."""
        if not self.doc or self._view_mode != ViewMode.SCROLL or not self._labels:
            return
        sb = self.scroll_area.verticalScrollBar()
        scroll_y = sb.value()
        vh = sb.pageStep()  # viewport height
        view_mid = scroll_y + vh // 2

        best_page = self._current_page; best_overlap = 0
        for pi, l in enumerate(self._labels):
            if not l.isVisible(): continue
            top = l.pos().y(); h = l.height(); bottom = top + h
            visible_top = max(top, scroll_y)
            visible_bottom = min(bottom, scroll_y + vh)
            visible = max(0, visible_bottom - visible_top)
            if visible > best_overlap:
                best_overlap = visible; best_page = pi

        if best_page != self._current_page and best_overlap > vh * 0.5:
            self._current_page = best_page; self._update_nav_ui()
    ...
    @staticmethod
    def _render_page(doc, page_idx, zoom_key, vw, vh, force_fit=False):
        import fitz
        page = doc[page_idx]; pw, ph = page.rect.width, page.rect.height
        if force_fit:
            zoom = min(vw / pw, vh / ph)
        elif zoom_key == "fw":
            zoom = vw / pw
        elif zoom_key == "fh":
            zoom = vh / ph
        ...
        return QPixmap.fromImage(qimg).scaled(
            tw, th, QtCore.AspectRatioMode.IgnoreAspectRatio,
    ...
    def _get_or_render(self, page_idx, vw, vh=99999, force_fit=False):
        key = (page_idx, self._zoom_key(vw, vh))
        if key in PdfReaderWidget._cache:
            return PdfReaderWidget._cache[key]
        pix = PdfReaderWidget._render_page(self.doc, page_idx, key[1], vw, vh, force_fit)
    ...
    def _on_fit_height(self): self.zoom_fit_height()
    ...
    def opened_from_file_list(self) -> bool:
        return getattr(self, '_btn_open_source', 'dialog') == 'file_list'
